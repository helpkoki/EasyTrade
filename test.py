...
app = Flask(__name__)
CORS(app)
model = MobileNetV2(weights="imagenet", include_top=True)  # Now using full model for object recognition
...
def predict_price():
    ...
    try:
        ...
        # Classify the image to get object class
        predictions = model.predict(x)
        from tensorflow.keras.applications.mobilenet_v2 import decode_predictions
        decoded_predictions = decode_predictions(predictions, top=1)[0]
        object_class = decoded_predictions[0][1]  # Get the class name
